...
document.head.children[2].innerText = "Hello AltCampus!";
// 2. Select the element using the children property:

//    - Select the `h1` element and change the value to `Learning DOM`
document.querySelector('h1').textContent =  "Learning DOM";
//    - Select the first `li` element inside the `ul` with class `topics` and change the innerText to `all about document`
document.querySelector('ul').children[0].innerText = `all about document`;
...
console.log(allTopics.forEach(textname => textname.innerText));
// 13. Select all the elements with class `list` and store in variable `listOfSelectedTopics`
allTopics.forEach(textname => console.log(textname.innerText));
...
let fielsetElm = document.querySelector("fieldset");
// 27. Select the parent element of the element stored in `fieldsetElm` variable (#5) and log the `typeof` the element.
console.log(typeof fielsetElm.parentElement);