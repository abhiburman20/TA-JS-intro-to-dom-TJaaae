...
allHrsArray.forEach((backgound) => (backgound.style.border = "1px solid tomato"));
// Change the background of all the hr to "antiquewhite" using for of loop.
for (let backgound of allHrsArray) {
  backgound.style.border = "antiquewhite";
}
// Change the 'border-radius' of all the hr to "5px" using array.
allHrsArray.forEach((borderRad) => (borderRad.style.borderRadius = "5px"));
// Change the alignment of the heading(h1) to center.
heading.style.textAlign = "center";
// Change the font size of the heading to 3rem.
heading.style.fontSize = "3rem";
// Change the border of hr with class 'image' to `2px solid purple`.
allHrsArray.filter((allarr) => allarr.className == "image").forEach((allarr) => (allarr.style.border = `2px solid purple`));
// Hide the box number 17 (last box).
document.querySelector(".seventeen").style.display = "none";
// Change the border of all the hr element from solid to dashed type
allHrsArray.forEach((allarr) => (allarr.style.borderStyle = "dashed"));
...
[...box1.children].forEach((allElement) => allElement.remove());
...
let box2newpara = document.createElement("p");
box2newpara.innerText = "Append inserts as last child";
document.querySelector(".two").append(box2newpara);
// Select box 3 and prepend a new paragraph element with content "Prepend inserts as first child" just before hr element.
let box3newpara = document.createElement("p");
box3newpara.innerText = "Prepend inserts as first child";
document.querySelector(".three").prepend(box3newpara);
...
[...document.querySelectorAll(".box")].filter((allElement, i) => i % 2 === 0).forEach((allElement) => (allElement.style.backgroundColor = "aliceblue"));
...
[...box7.children].forEach((allElement) => allElement.remove());
